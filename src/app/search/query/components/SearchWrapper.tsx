...
function Selector({
  ...
  const initlizeTime = () => {
    ...
    if (almost(diff, oneDay)) {
      ...
      setRangeValue([dayjs(start / 1000), dayjs(end / 1000)]);
    }
  ...
  useEffect(() => {
    ...
    let startTime: number | null = query.startTime ? query.startTime : null;
    let endTime: number | null = query.endTime ? query.endTime : null;
    switch (time) {
      case "all":
        startTime = null;
        break;
